...
function twoProductResidue(x: number, y: number, product: number): number {
 ...
 */
export function sum(values: Float64Array, start = 0, count = values.length - start): number {
  if (count === 0) {
 ...
 * equal to `value`.
 */
export function searchSortedRight(sorted: ArrayLike<number>, value: number): number {
  let low = 0;
 ...
 */
export function argSortTopN(values: ArrayLike<number>, count: number): number[] {
  const best: number[] = [];
