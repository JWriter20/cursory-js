...
import { searchSortedRight } from '../numeric';
import { createPcg64, Pcg64 } from './pcg64';
import {
  FI_DOUBLE,
  KI_DOUBLE,
  WI_DOUBLE,
...
export class Generator {
  constructor(private readonly bitGenerator: Pcg64) {}
  ...
  uniformArray(low: number, high: number, size: number): Float64Array {
    const range = high - low;
    if (!Number.isFinite(range)) {
      throw new RangeError('Range exceeds valid bounds');
    }
  ...
  standardNormalArray(size: number): Float64Array {
    const out = new Float64Array(size);
    for (let i = 0; i < size; i += 1) {
  ...
  private boundedLemire32(range: number): number {
  ...
  choice(weights: Float64Array): number {
...
export function defaultGenerator(seed?: number | bigint): Generator {
