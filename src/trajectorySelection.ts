...
import { LOADED_TRAJECTORIES, Trajectory } from './data';
import { argSortTopN, EPSILON, hypot, searchSortedRight, sum } from './numeric';
import { Generator } from './random/generator';

/** An (x, y) position in screen pixels. */
...
function endpointDelta(trajectory: Trajectory, axis: 0 | 1): number {
...
export interface NearestOptions {
  /** Weight of direction similarity in the combined score. */
  directionWeight?: number;
  /** Weight of length similarity in the combined score. */
  lengthWeight?: number;
  /** How many of the nearest recordings to return. */
  topN?: number;
}
...
export function findNearestTrajectory(
  targetStart: Point,
  targetEnd: Point,
  { directionWeight = 0.8, lengthWeight = 0.2, topN = 5 }: NearestOptions = {},
): Trajectory[] {
...
export interface ClosestOptions {
  /** How many nearest recordings each query contributes. */
  numNearestToSample?: number;
  /** How many perturbed re-queries widen the candidate pool. */
  randomSampleIterations?: number;
  /** Relative preference for shorter, straighter paths, from 0 to 1. */
  directness?: number;
}

export interface ClosestTrajectory {
  trajectory: Trajectory;
  /** Target x displacement. */
  dxTarget: number;
  /** Target y displacement. */
  dyTarget: number;
  /** Target straight-line distance. */
  lengthTarget: number;
}
...
export function morphTrajectory(
  ...
  const morphed: Point[] = points.map((point) => {
 ...
 */
export function jitterTrajectory(
  points: readonly Point[],
  ...
  const jittered: Point[] = new Array(count);
  let lateralOffset = 0;
  for (let i = 0; i < count; i += 1) {
    lateralOffset =
 ...
 */
export function generateMiddleBiasedPoint(
  x1: number,
  y1: number,
  x2: number,
  y2: number,
...
function isWholePixelPath(points: readonly Point[]): boolean {
...
export interface FoundTrajectory {
  points: Point[];
  /** Timestamps of the recording the path came from, in milliseconds. */
  timings: number[];
}
